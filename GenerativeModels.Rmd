...
train_qda <- train(y ~ ., method = "lda", data = mnist_27$train)
y_hat <- predict(train_qda, mnist_27$test)
confusionMatrix(data = y_hat, reference = mnist_27$test$y)
...
train_set %>% mutate(y = factor(y)) %>% ggplot(aes(x_1,x_2, fill = y, color =y)) + geom_point(show.legend = FALSE) +
...
t(train_lda$finalModel$means) %>% data.frame() %>%
  mutate(predict_name = rownames(.)) %>%
  ggplot(aes(cerebellum,hippocampus, label = predict_name)) +
  geom_point() +
  geom_text() +
  geom_abline()
