...
library.list = c("ggplot2", "readr", "corrgram", "plotly", "rpart", 
                      "tidyr", "tree", "reshape2", "corrplot")
lapply (library.list, library, character.only=TRUE)
```
...
Plot.Dis <- function (data) {
  ggplot(data, aes(x = target,fill = target, color = target)) + 
  geom_bar() + 
...
data.all[c(1:12,31)] %>% 
  pivot_longer(cols = -c(target), names_to = "var", values_to = "value") %>%
